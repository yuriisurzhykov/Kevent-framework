...
/**
 *  Interface representing a EventBus, a bus system for managing and dispatching events. Using
 *  this `EventBus` you can subscribe your object to listen a set of events. Every event, that to
 *  be sent may be sticky or non-sticky(regular). You can subscribe any class for event if you
 *  have a CoroutineScope. For the UI part the `viewModelScope` may be used.
 */
interface EventBus : EventManager {
    ...
    fun <T : Event> subscribe(
        ...
        errorHandler: suspend (Throwable) -> Unit = { throw it }
    )
     ...
     *  is to subscribe object to the [Event] through [SharedFlow]
     */
    abstract class Abstract(
        ...
        override fun <T : Event> subscribe(
            ...
            flowCommunication.asSharedFlow()
                ...
                .catch { throwable -> errorHandler.invoke(throwable) }
                .launchIn(scope)
        }
         ...
         *
         * If event has not been published the EventBus looks for persisted event and
